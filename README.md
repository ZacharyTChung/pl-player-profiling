...
| `elasticity` | 5 s | Bootstrap intervals, per-league refits, functional form, sweeper check |
| `ablation` | 20 min | Leave-one-family-out and keep-one-only, 52 cells, 25 simulations each |
| `outcomes` | 1 min | Composition against real points and goal difference, out of sample |
...
src/elasticity.py         bootstrap intervals, per-league refits, sweeper exposure check
src/ablation.py           feature-family ablation on the primary sample
src/outcomes.py           the two modes against real results, out of sample
...
12. **The modes survive removing any feature family, and only territory carries them
    ...
    ball.
13. **The modes predict real results; the archetypes do not.** Over 490 team-seasons with
    ...
    have. Bayern 2018-19, Real Madrid 2020-21 and City 2021-22 sit at the top of the attacking
    share; Sheffield United 2020-21 near the bottom.
14. **Outliers are the players the listing system also declines to place.** 11 of the 15
